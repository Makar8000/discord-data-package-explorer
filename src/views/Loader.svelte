<script>
    ...
    let loading = false;
    let error = false;
    let topCount = localStorage.getItem('topCount') || null;
    if(!topCount) {
        const urlParams = new URLSearchParams(window.location.search);
        topCount = urlParams.get('topCount');
        topCount = topCount ? Number.parseInt(topCount) : 10;
        localStorage.setItem('topCount', topCount);
    }
    ...
    function onTopCountChange (event) {
        let value = event && event.target && event.target.value ? event.target.value : 10;
        if(value < 10)
            value = 10;
        if(value > 500)
            value = 500;
        localStorage.setItem('topCount', value);
    }
...
<template>
    <div class="app-loader">
        <div class="app-loader-boxes">
            ...
            <div class="app-loader-settings">
                <small class="app-loader-tag tag">2</small>
                <div class="app-loader-settings-title">Set your settings</div>
                <br />
                <div class="app-loader-settings-opts">
                  <span></span>
                  <span>Top Count:</span>
                  <input type="number" value={topCount} min="10" max="500" on:change="{onTopCountChange}" />
                  <span></span>
                </div>
            </div>
            <div class="app-loader-upload" on:click="{filePopup}" style="cursor: { loading ? '' : 'pointer' }" on:drop="{handleDrop}" on:dragover="{handleDragOver}">
                <small class="app-loader-tag tag">3</small>
                <label for="upload">
...
<style lang="scss">
  ...
  .app-loader-settings {
    ...
    color: var(--secondary-color);
		font-weight: 400;
		border-radius: 0.3rem;
    .app-loader-settings-title {
      text-align: center;
    }
    .app-loader-settings-opts {
      display: flex;
      flex-direction: row;
      margin-left: 2rem;
      margin-right: 2rem;
